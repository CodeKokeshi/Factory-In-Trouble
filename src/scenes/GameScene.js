...
import { createVictoryOverlay } from '../ui/victoryOverlay';
...
const STEAM_HAZARD_INTRO_LEVEL_INDEX = 6;
const STEAM_HAZARD_MIN_COOLDOWN_MS = 11000;
const STEAM_HAZARD_MAX_COOLDOWN_MS = 17500;
const STEAM_HAZARD_RETRY_MIN_MS = 1600;
const STEAM_HAZARD_RETRY_MAX_MS = 3200;
const STEAM_HAZARD_CHEST_CLEAR_SLOTS = 2;
const STEAM_HAZARD_RISE_DURATION_MS = 520;
const STEAM_HAZARD_VISIBLE_DURATION_MS = 3200;
const STEAM_HAZARD_FALL_DURATION_MS = 480;
const STEAM_HAZARD_TUTORIAL_DISABLE_STORAGE_KEY = 'machines_phaser_steam_hazard_tutorial_disabled_v1';
const STEAM_HAZARD_TUTORIAL_BUTTON_LOCK_MS = 1000;
...
export default class GameScene extends Phaser.Scene {
  constructor() {
    ...
    this.steamHazardTimerMs = 0;
    this.steamCloudActive = false;
    this.steamCloudGraphics = null;
    this.steamCloudTweens = [];
    this.steamHazardVfxList = [];
    this.steamCloudRunToken = 0;
    this.steamTutorialShownThisRun = false;
    this.steamTutorialActive = false;
    this.steamTutorialOverlay = null;
    this.steamTutorialPreviewState = null;
    this.steamTutorialButtonUnlockEvent = null;
    this.steamTutorialPendingLaneId = null;
    this.steamTutorialPreferenceLoaded = false;
    this.steamTutorialDoNotShowAgain = false;
    ...
    this.pauseMenuFailSafeEvent = null;
  ...
  applyLevelConfig(levelConfig) {
    ...
    const clawBreakdownEndlessEnabled = this.levelMode === 'endless' && this.levelId !== DEBUG_LEVEL_ID;
  ...
  resetRunState() {
    ...
    this.cleanupSteamHazard();
    this.cleanupSteamHazardTutorial({ resumeGameplay: false });
    ...
    this.maxComboReached = 1;
    this.jamCount = 0;
    this.levelStartTimeMs = this.time?.now ?? 0;
    this.comboMilestoneTier = 0;
    ...
    this.steamTutorialOverlay = null;
    this.steamTutorialPreviewState = null;
    if (this.steamTutorialButtonUnlockEvent) {
      this.steamTutorialButtonUnlockEvent.remove(false);
      this.steamTutorialButtonUnlockEvent = null;
    }
    this.steamTutorialPendingLaneId = null;
    ...
    if (this.isSteamHazardLevelEnabled()) {
      this.resetSteamHazardTimer();
    } else {
      this.steamHazardTimerMs = Number.POSITIVE_INFINITY;
    }
  ...
  handleGameplayKeyDown(event) {
    ...
    if (key === 'escape' || event?.code === 'Escape') {
      if (this.pauseTransitionLock || this.isGameOver || this.levelComplete || this.sceneTransitioning || this.isDraftActive) {
        return;
      }
    ...
    if (isCampaignBracketProgressHotkeyEvent(event)) {
      ...
      const result = cycleCampaignProgressBracketHotkey(getCampaignLevels().map((entry) => entry.id));
      this.emitFloatingText(640, 120, result.message, '#fef9c3', 22);
      this.playSfx('transfer', 0.85);
      return;
    }
  ...
  resetTutorialSuppressionFlags() {
    ...
    this.setSteamHazardTutorialDisabled(false);
    this.droneTutorialShownThisRun = false;
    ...
    this.clawBreakTutorialDoNotShowAgain = false;
    this.steamTutorialShownThisRun = false;
    this.steamTutorialPreferenceLoaded = true;
    this.steamTutorialDoNotShowAgain = false;
  ...
  clearPauseMenuFailSafe() {
    if (this.pauseMenuFailSafeEvent) {
      this.pauseMenuFailSafeEvent.remove(false);
      this.pauseMenuFailSafeEvent = null;
    }
  }
  ...
  handlePauseButtonPressed() {
    ...
    this.clearPauseMenuFailSafe();
    this.pauseMenuFailSafeEvent = this.time.delayedCall(320, () => {
      this.pauseMenuFailSafeEvent = null;
      if (!this.isPaused || this.sceneTransitioning || this.isGameOver || this.levelComplete) {
        this.pauseTransitionLock = false;
        return;
      }

      if (!this.pauseMenuContainer?.active) {
        this.openPauseMenu();
      }
      this.pauseTransitionLock = false;
    });
  ...
  applyComboGain(baseGain) {
    const gainSteps = Math.max(1, Math.floor(baseGain || 1));
    let nextMultiplier = this.multiplier;

    for (let step = 0; step < gainSteps; step += 1) {
      nextMultiplier += nextMultiplier < 20 ? 1 : 0.5;
    }

    this.multiplier = Math.round(nextMultiplier * 2) / 2;
    this.maxComboReached = Math.max(this.maxComboReached, this.multiplier);
  }

  getElapsedLevelSeconds() {
    const nowMs = this.time?.now ?? this.levelStartTimeMs;
    const elapsedMs = Math.max(0, nowMs - this.levelStartTimeMs);
  ...
  formatDurationSeconds(seconds) {
    ...
    const remainder = safeSeconds - wholeMinutes * 60;
    const paddedRemainder = remainder.toFixed(1).padStart(4, '0');
    return `${wholeMinutes}:${paddedRemainder}`;
  }
  ...
  refreshScoreUi() {
    ...
    if (this.multiplierText) {
      this.multiplierText.setText(`x${this.formatComboValue(this.multiplier)}`);
    }
  }
  ...
  handleComboConsume(consumedItem = null) {
    ...
    this.applyComboGain(multiplierGain);
    ...
    if (milestoneTier > this.comboMilestoneTier) {
      ...
      this.emitFloatingText(640, 236, `COMBO x${this.formatComboValue(this.multiplier)}!`, '#67e8f9', 30);
  ...
  update(_time, delta) {
    ...
    this.updateSteamHazard(scaledDeltaMs);
  ...
  triggerLevelComplete() {
    ...
    const victoryOverlay = createVictoryOverlay(this, {
      levelName: this.levelName,
      acceptedCount: this.acceptedCount,
      levelQuota: this.levelQuota,
      score: this.score,
      maxCombo: this.formatComboValue(this.maxComboReached),
      jamCount: this.jamCount,
      finishTime: this.formatDurationSeconds(this.getElapsedLevelSeconds()),
      hasNextLevel: Boolean(nextLevelId),
      onNextLevel: transitionToNextLevel,
      onLevelSelect: transitionToSelection,
      fonts: {
        display: GAME_DISPLAY_FONT,
        ui: GAME_UI_FONT
      }
    });

    this.levelResultOverlay = victoryOverlay.overlay;
    this.levelResultText = victoryOverlay.levelNameText;
  }
  ...
  shouldShowClawBreakdownTutorial() {
    ...
    return this.clawBreakdownEnabled === true;
  }

  isClawBreakdownTutorialBlockedNow() {
    return (
      this.sceneTransitioning
      || this.isGameOver
      || this.levelComplete
      ...
      || this.steamTutorialActive
      || this.laneSwapActive
    );
  }

  tryStartPendingClawBreakdownTutorial() {
    if (!this.shouldShowClawBreakdownTutorial() || this.isClawBreakdownTutorialBlockedNow()) {
      return false;
    }

    const pendingLane = this.lanesById?.[this.clawBreakTutorialPendingLaneId] || null;
    if (!pendingLane?.clawOverheated) {
      this.clawBreakTutorialPendingLaneId = null;
      return false;
    }

    return this.startClawBreakdownTutorial(pendingLane);
  }
  ...
  setSteamHazardTutorialDisabled(disabled) {
    this.steamTutorialPreferenceLoaded = true;
    this.steamTutorialDoNotShowAgain = disabled === true;
    ...
    try {
      if (this.steamTutorialDoNotShowAgain) {
        window.localStorage.setItem(STEAM_HAZARD_TUTORIAL_DISABLE_STORAGE_KEY, '1');
      } else {
        window.localStorage.removeItem(STEAM_HAZARD_TUTORIAL_DISABLE_STORAGE_KEY);
      }
    } catch {
      // Ignore storage write failures.
    }
  ...
  startClawBreakdownTutorial(lane) {
    ...
    if (this.isClawBreakdownTutorialBlockedNow()) {
      return false;
    }

    const activeLane = lane?.clawOverheated
      ? lane
      : (this.lanesById?.[this.clawBreakTutorialPendingLaneId] || null);
    if (!activeLane?.clawOverheated) {
      this.clawBreakTutorialPendingLaneId = null;
      return false;
    }
    ...
    this.clawBreakTutorialPendingLaneId = activeLane.id;
    ...
    const laneLabel = String(activeLane?.id || 'lane').replace(/_/g, ' ').toUpperCase();
  ...
  updateClawBreakdowns(deltaMs) {
    ...
    if (this.clawBreakTutorialPendingLaneId && !this.clawBreakTutorialActive) {
      this.tryStartPendingClawBreakdownTutorial();
    }
  ...
  isDroneSabotageLevelEnabled() {
    ...
    return (
      Number.isFinite(levelIndex)
      && levelIndex >= DRONE_SABOTAGE_INTRO_LEVEL_INDEX
      && levelIndex < STEAM_HAZARD_INTRO_LEVEL_INDEX
    );
  }

  shouldShowDroneIncidentTutorial() {
    this.loadDroneIncidentTutorialPreference();
    if (this.droneTutorialDoNotShowAgain) {
      return false;
    }
  ...
  loadDroneIncidentTutorialPreference() {
    if (this.droneTutorialPreferenceLoaded) {
      return;
    }
    ...
    try {
      const raw = window.localStorage.getItem(DRONE_TUTORIAL_DISABLE_STORAGE_KEY);
      this.droneTutorialDoNotShowAgain = raw === '1' || raw === 'true';
    } catch {
      this.droneTutorialDoNotShowAgain = false;
    }
  }

  setDroneIncidentTutorialDisabled(disabled) {
    this.droneTutorialPreferenceLoaded = true;
    this.droneTutorialDoNotShowAgain = disabled === true;
    ...
    try {
      if (this.droneTutorialDoNotShowAgain) {
        window.localStorage.setItem(DRONE_TUTORIAL_DISABLE_STORAGE_KEY, '1');
      } else {
        window.localStorage.removeItem(DRONE_TUTORIAL_DISABLE_STORAGE_KEY);
      }
    } catch {
      // Ignore storage write failures.
    }
  ...
  cleanupDroneIncidentTutorial({ resumeGameplay = false } = {}) {
    const wasActive = this.droneTutorialActive;
    this.droneTutorialLoopToken += 1;

    if (this.droneTutorialButtonUnlockEvent) {
      this.droneTutorialButtonUnlockEvent.remove(false);
      this.droneTutorialButtonUnlockEvent = null;
    }
  ...
  startDroneIncidentTutorial(targetItemId, dropPlan) {
    ...
    const title = this.add
      .text(panelX, panelY - 234, 'DRONE INCIDENT', {
        fontFamily: GAME_DISPLAY_FONT,
        fontSize: '44px',
        color: '#fff1e6',
        align: 'center'
    ...
    const wrongItem = this.add.circle(wrongDropX, wrongLaneY, 12, 0xef4444, 1).setStrokeStyle(2, 0x7f1d1d, 1).setAlpha(0);
    const wrongLabel = this.add
      .text(wrongDropX, wrongLaneY - 28, 'SABOTAGED', {
        fontFamily: GAME_UI_FONT,
        fontSize: '16px',
        color: '#fecaca'
      })
    ...
    const description = this.add
      .text(
        640,
        500,
        'Drones steal a correctly sorted item and drop it into the wrong lane.',
        {
          fontFamily: GAME_UI_FONT,
          fontSize: '24px',
          color: '#fde68a',
    ...
    let doNotShowAgain = false;
    const checkboxContainer = this.add.container(640, 538).setDepth(374);
    const checkboxBody = this.add.rectangle(-96, 0, 22, 22, 0x0f172a, 0.95).setStrokeStyle(2, 0x94a3b8, 0.95);
    const checkboxFill = this.add.rectangle(-96, 0, 12, 12, 0x86efac, 1).setVisible(false);
    const checkboxLabel = this.add
      .text(16, 0, 'Do not show again', {
    ...
    checkboxContainer.add([checkboxBody, checkboxFill, checkboxLabel, checkboxHit]);
    overlay.add(checkboxContainer);
    syncCheckboxVisual();

    this.pauseTransitionLock = true;
    this.createPauseMenuEntry(
      overlay,
      'Understood',
      596,
      0x86efac,
      () => {
        this.setDroneIncidentTutorialDisabled(doNotShowAgain);
        this.pauseTransitionLock = false;
    ...
    this.droneTutorialButtonUnlockEvent = this.time.delayedCall(DRONE_TUTORIAL_BUTTON_LOCK_MS, () => {
      this.droneTutorialButtonUnlockEvent = null;
  ...
  updateDroneSabotage(deltaMs) {
    if (!this.isDroneSabotageLevelEnabled() || this.laneSwapActive || deltaMs <= 0) {
      return;
    }

    if (this.shouldSuspendDroneSabotage()) {
      if (this.droneActive) {
        this.cleanupDroneSabotage(true);
        this.resetDroneSabotageTimer(DRONE_SABOTAGE_RETRY_MIN_MS, DRONE_SABOTAGE_RETRY_MAX_MS);
      }
  ...
  startDroneSabotageRun(targetItemId, plannedDrop) {
    if (this.droneActive || this.shouldSuspendDroneSabotage()) {
      return;
    }
  ...
  isSteamHazardLevelEnabled() {
    ...
    const levelIndex = Number(this.levelConfig?.index);
    return Number.isFinite(levelIndex) && levelIndex >= STEAM_HAZARD_INTRO_LEVEL_INDEX;
  }

  shouldSuspendSteamHazard() {
    return (
      this.isPaused
      || this.isDraftActive
      || this.isGameOver
  ...
  getSteamHazardObscureWorldBounds(lane) {
    if (!lane) {
      return null;
    }

    const spacing = this.getActiveSideSpacing();
    const clearLaneSpan = STEAM_HAZARD_CHEST_CLEAR_SLOTS * spacing;
    const clearStartLanePos = Math.max(0, lane.length - clearLaneSpan);
    const xIntake = lane.intakeX;
    const xClearEdge = lane.intakeX + lane.direction * clearStartLanePos;
    const fogLeft = Math.min(xIntake, xClearEdge);
    const fogRight = Math.max(xIntake, xClearEdge);
    const width = Math.max(28, fogRight - fogLeft);
    const centerX = (fogLeft + fogRight) * 0.5;
    const padY = 16;
    const height = SIDE_BELT_HEIGHT + padY * 2;

    return { centerX, laneY: lane.y, width, height };
  }

  updateSteamHazard(deltaMs) {
    if (!this.isSteamHazardLevelEnabled() || this.laneSwapActive || deltaMs <= 0) {
      return;
    }

    if (this.shouldSuspendSteamHazard()) {
      if (this.steamCloudActive) {
        this.cleanupSteamHazard();
        this.resetSteamHazardTimer(STEAM_HAZARD_RETRY_MIN_MS, STEAM_HAZARD_RETRY_MAX_MS);
      }
      return;
    }
    ...
    this.steamHazardTimerMs -= deltaMs;
    if (this.steamHazardTimerMs > 0) {
      return;
    }

    const targetLane = this.pickSteamTargetLane();
    if (!targetLane) {
      this.resetSteamHazardTimer(STEAM_HAZARD_RETRY_MIN_MS, STEAM_HAZARD_RETRY_MAX_MS);
      return;
    }

    if (this.shouldShowSteamHazardTutorial()) {
      const startedTutorial = this.startSteamHazardTutorial(targetLane);
      if (startedTutorial) {
        return;
      }
    }

    this.startSteamCloud(targetLane);
  }

  shouldShowSteamHazardTutorial() {
    this.loadSteamHazardTutorialPreference();
    if (this.steamTutorialDoNotShowAgain) {
      return false;
    }

    if (this.steamTutorialShownThisRun || this.steamTutorialActive) {
      return false;
    }
  ...
  loadSteamHazardTutorialPreference() {
    if (this.steamTutorialPreferenceLoaded) {
      return;
    }
    ...
    try {
      const raw = window.localStorage.getItem(STEAM_HAZARD_TUTORIAL_DISABLE_STORAGE_KEY);
      this.steamTutorialDoNotShowAgain = raw === '1' || raw === 'true';
    } catch {
      // Ignore storage read failures.
    }
  }

  acknowledgeSteamHazardTutorial() {
    const pendingLaneId = this.steamTutorialPendingLaneId;
    this.cleanupSteamHazardTutorial({ resumeGameplay: true });

    if (this.sceneTransitioning || this.isGameOver || this.levelComplete) {
      return;
    }

    const lane = this.lanesById[pendingLaneId];
    if (lane) {
      this.startSteamCloud(lane);
      return;
    }
  ...
  cleanupSteamHazardTutorial({ resumeGameplay = false } = {}) {
    const wasActive = this.steamTutorialActive;
    ...
    if (this.steamTutorialPreviewState) {
      const { animatedTargets = [], animatedTweens = [] } = this.steamTutorialPreviewState;
      animatedTweens.forEach((tween) => {
        tween?.remove?.();
      });
    ...
    if (this.steamTutorialOverlay?.active) {
      this.tweens.killTweensOf(this.steamTutorialOverlay);
      this.steamTutorialOverlay.destroy(true);
    }
    this.steamTutorialOverlay = null;
    this.steamTutorialPendingLaneId = null;
    this.steamTutorialActive = false;
  ...
  startSteamHazardTutorial(targetLane) {
    if (this.steamTutorialActive || !this.shouldShowSteamHazardTutorial() || !targetLane) {
      return false;
    }
    ...
    this.steamTutorialShownThisRun = true;
    this.steamTutorialActive = true;
    this.steamTutorialPendingLaneId = targetLane.id;
    ...
    const panelY = 362;
    const panelWidth = 892;
    const panelHeight = 518;

    const panelShadow = this.add.rectangle(panelX, panelY + 9, panelWidth, panelHeight, 0x020617, 0.56);
    const panel = this.add
      .rectangle(panelX, panelY, panelWidth, panelHeight, 0x4a2012, 0.97)
      .setStrokeStyle(2, 0xf0bd85, 0.95);
    const panelTopStrip = this.add.rectangle(panelX, panelY - 219, panelWidth - 74, 40, 0xffffff, 0.14);

    const title = this.add
      .text(panelX, panelY - 218, 'STEAM BURST', {
        fontFamily: GAME_DISPLAY_FONT,
        fontSize: '46px',
        color: '#fff1e6',
        align: 'center'
    ...
    const subtitle = this.add
      .text(panelX, panelY - 182, 'Keep an eye on the intake — chest claws stay readable.', {
        fontFamily: GAME_UI_FONT,
        fontSize: '22px',
        color: '#fde68a',
        align: 'center',
    ...
    const previewY = 302;
    const previewShadow = this.add.rectangle(640, previewY + 6, 760, 166, 0x020617, 0.48);
    const previewFrame = this.add
      .rectangle(640, previewY, 760, 166, 0x1f2937, 0.94)
      .setStrokeStyle(2, 0xf0bd85, 0.72);

    const previewContainer = this.add.container(640, previewY).setDepth(372);
    const belt = this.add.rectangle(0, 14, 706, 34, 0x3f3f46, 0.98).setStrokeStyle(2, 0x0f172a, 0.7);
    const stripes = [];
    for (let stripeX = -332; stripeX <= 332; stripeX += 46) {
      stripes.push(this.add.rectangle(stripeX, 14, 22, 4, 0xa1a1aa, 0.34));
    }

    const steamMask = this.add
      .rectangle(-118, 14, 404, 54, 0xe2e8f0, 0.38)
      .setBlendMode(Phaser.BlendModes.SCREEN);
    const steamDots = [-268, -220, -176, -132, -88].map((x) =>
      this.add.circle(x, 8, Phaser.Math.Between(11, 18), 0xf8fafc, 0.55)
    );
    const clearChip = this.add
      .rectangle(248, 14, 168, 42, 0x0f172a, 0.12)
      .setStrokeStyle(2, 0x86efac, 0.55);
    const clawLine = this.add.graphics();
    clawLine.lineStyle(3, 0xfde68a, 0.95);
    clawLine.lineBetween(312, 6, 296, 18);
    clawLine.lineBetween(296, 18, 312, 24);
    const clearLabel = this.add
      .text(248, -52, 'CLEAR BY CLAWS', {
        fontFamily: GAME_UI_FONT,
        fontSize: '18px',
        color: '#bbf7d0',
        align: 'center'
    ...
    previewContainer.add([belt, ...stripes, steamMask, ...steamDots, clearChip, clawLine, clearLabel]);
    ...
    const steamPulse = this.tweens.add({
      targets: steamDots,
    ...
    const steamMaskTween = this.tweens.add({
      targets: steamMask,
...
// 1. Moved text up to sit right below the preview box
// 1. Moved text up to sit right below the preview box
    const helperLineOne = this.add
      .text(panelX, panelY + 55, 'Bursts billow across the belt and hide item silhouettes.', {
        fontFamily: GAME_UI_FONT,
        fontSize: '26px',
        color: '#f8fafc',
        align: 'center',
    ...
    const helperLineTwo = this.add
      .text(panelX, panelY + 95, 'The last two belt slots beside each chest stay visible.', {
        fontFamily: GAME_UI_FONT,
        fontSize: '26px',
        color: '#fde68a',
        align: 'center',
    ...
    // 2. Shifted checkbox down to sit between the text and the button
    let doNotShowAgain = false;
    const checkboxContainer = this.add.container(640, 505).setDepth(374); // Changed from 492
    const checkboxBody = this.add.rectangle(-96, 0, 22, 22, 0x0f172a, 0.95).setStrokeStyle(2, 0x94a3b8, 0.95);
    const checkboxFill = this.add.rectangle(-96, 0, 12, 12, 0x86efac, 1).setVisible(false);
    const checkboxLabel = this.add
      .text(16, 0, 'Do not show again', {
    ...
    const acknowledgeTutorial = () => {
      this.setSteamHazardTutorialDisabled(doNotShowAgain);
      this.pauseTransitionLock = false;
      this.acknowledgeSteamHazardTutorial();
    };
    ...
    this.steamTutorialPreviewState = {
      animatedTargets: [...steamDots, steamMask, clawLine],
      animatedTweens: [stripeTween, steamPulse, steamMaskTween]
    };

    // 3. Shifted button down to clear the checkbox area completely
    let understoodButton = this.createPauseMenuEntry(
      overlay,
      'Understood',
      575, // Changed from 554
      0x9ca3af,
      acknowledgeTutorial,
      disabledButtonStyle
    );

    this.steamTutorialButtonUnlockEvent = this.time.delayedCall(STEAM_HAZARD_TUTORIAL_BUTTON_LOCK_MS, () => {
      this.steamTutorialButtonUnlockEvent = null;
      if (!this.steamTutorialActive || this.steamTutorialOverlay !== overlay || !overlay.active) {
        return;
      }
    ...
    this.steamTutorialOverlay = overlay;
    return true;
  }

  pickSteamTargetLane() {
    const validLanes = this.laneLayout
      .map((laneConfig) => this.lanesById[laneConfig.id])
      .filter((lane) => lane && this.canEnterLane(lane.id));

    if (validLanes.length === 0) {
      return null;
    }

    return Phaser.Utils.Array.GetRandom(validLanes);
  }

  teardownSteamHazardVfx() {
    if (Array.isArray(this.steamCloudTweens)) {
      this.steamCloudTweens.forEach((tween) => tween?.remove?.());
    }
    this.steamCloudTweens = [];

    if (Array.isArray(this.steamHazardVfxList)) {
      this.steamHazardVfxList.forEach((obj) => {
  ...
  startSteamCloud(targetLane) {
    if (!targetLane || this.steamCloudActive) {
      return;
    }

    const bounds = this.getSteamHazardObscureWorldBounds(targetLane);
    if (!bounds || !Number.isFinite(bounds.width) || bounds.width < 20) {
      this.resetSteamHazardTimer(STEAM_HAZARD_RETRY_MIN_MS, STEAM_HAZARD_RETRY_MAX_MS);
      return;
    }

    this.steamCloudActive = true;
    this.steamCloudRunToken += 1;
    const runToken = this.steamCloudRunToken;
    this.steamCloudTweens = [];
    this.steamHazardVfxList = [];

    const particleKey = 'fx_dot';
    const { centerX, laneY, width, height } = bounds;
    
    // INCREASE DEPTH: Make sure this is higher than the food items' depth (e.g., 100 or 150)
    // but lower than your UI depth (which is ~360+)
    const fogDepth = 150;

    const fogRect = this.add
      .rectangle(centerX, laneY, width * 1.04, height, 0xd4dce6, 0)
      .setStrokeStyle(2, 0xf8fafc, 0.2)
      .setDepth(fogDepth);
    this.steamHazardVfxList.push(fogRect);

    const addSteamEmitter = (blendMode, frequency, quantity, gravityY, scaleStart, scaleEnd) => {
      if (!this.textures.exists(particleKey)) {
        return null;
      }

      const emitter = this.add.particles(centerX, laneY, particleKey, {
        blendMode,
        ...
        emitZone: {
          type: 'random',
          source: new Phaser.Geom.Rectangle(-width * 0.5, -height * 0.5, width, height)
        },
        tint: [0xffffff, 0xf1f5f9, 0xcbd5e1, 0x94a3b8]
      });
      emitter.setDepth(fogDepth + 1);
      this.steamHazardVfxList.push(emitter);
      return emitter;
    };

    addSteamEmitter('SCREEN', 16, 6, -32, 1.05, 3.4);
    addSteamEmitter('NORMAL', 30, 4, -12, 0.68, 2.2);

    const killIfStale = () => runToken !== this.steamCloudRunToken || !this.steamCloudActive;

    const riseTween = this.tweens.add({
      targets: fogRect,
      alpha: { from: 0, to: 0.58 },
      duration: STEAM_HAZARD_RISE_DURATION_MS,
      ease: 'Quad.Out',
      onComplete: () => {
        if (killIfStale()) {
          return;
        ...
        this.time.delayedCall(STEAM_HAZARD_VISIBLE_DURATION_MS, () => {
          if (killIfStale()) {
            return;
          }
          ...
          const fallTween = this.tweens.add({
            targets: fogRect,
            alpha: 0,
            duration: STEAM_HAZARD_FALL_DURATION_MS,
            ease: 'Quad.In',
            onStart: () => {
              this.steamHazardVfxList.forEach((obj) => {
                if (obj && obj !== fogRect && typeof obj.stop === 'function') {
                  obj.stop();
                }
              });
            },
            onComplete: () => {
              if (runToken !== this.steamCloudRunToken) {
                return;
              }
              this.teardownSteamHazardVfx();
              this.resetSteamHazardTimer();
            }
          });
          this.steamCloudTweens.push(fallTween);
        });
      }
    });
    this.steamCloudTweens.push(riseTween);
  }

  resetSteamHazardTimer(minMs = STEAM_HAZARD_MIN_COOLDOWN_MS, maxMs = STEAM_HAZARD_MAX_COOLDOWN_MS) {
    this.steamHazardTimerMs = Phaser.Math.Between(minMs, maxMs);
  }
  ...
  animateLaneItemIntoChest(item, lane, awardScore = true) {
    ...
    const finalizeAccept = () => {
      ...
      if (awardScore && this.multiplier >= 3) {
        this.emitFloatingText(lane.chestX, lane.y - 34, `CHAIN x${this.formatComboValue(this.multiplier)}`, '#34d399', 16);
